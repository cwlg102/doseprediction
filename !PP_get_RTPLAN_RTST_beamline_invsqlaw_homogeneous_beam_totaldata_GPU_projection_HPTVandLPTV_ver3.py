...
def beam_ray_tracing_no_multiprocess(line_draw_arr, area_co, beam_source_ras, ptv_arr, MV, spacing):
    ...
    # beam source ras - 빔 소스 좌표 (복셀화된 )
    # area_co - PTV 사영면의 좌표들 (복셀화된)
...
def beam_ray_tracing(dset):
    ...
    return line_draw_arr
def resample_by_size(img, wanted_xy, wanted_z, is_label=False):
    ...
    # wanted_xy = 128
    # wanted_z = 64
    # Define the target voxel size (e.g., 2mm x 2mm x 2mm)
    target_spacing = [spacing[0]*(shape[0]/wanted_xy), spacing[1]*(shape[1]/wanted_xy), spacing[-1]*(shape[-1]/wanted_z)]
    # Compute the resampling factor for each dimension
    resampling_factor = [curr / target for curr, target in zip(spacing, target_spacing)]

    resampler = sitk.ResampleImageFilter()
    resampler.SetOutputSpacing(target_spacing)
    resampler.SetSize([int(round(sz * rf)) for sz, rf in zip(img.GetSize(), resampling_factor)])
    resampler.SetOutputDirection(img.GetDirection())
    resampler.SetOutputOrigin(img.GetOrigin())
    if is_label==True:
        resampler.SetInterpolator(sitk.sitkNearestNeighbor)# only if segmentation
    else:
        resampler.SetInterpolator(sitk.sitkLinear) # image
    # Perform resampling
...
if __name__ == "__main__":
    ...
    xl_path = r"X:\!project\!2025_doseprediction\data_acquisition_chart.xlsx"
    dsc_wb = openpyxl.load_workbook(r"X:\!project\!2025_doseprediction\abdominal_dose_patients_cl_update.xlsx")
    dsc_ws = dsc_wb["Sheet"]
    ...
    for idx in range(100):
        ...
        hptv_name = ws.cell(idx+1, 3).value
        lptv_name = ws.cell(idx+1, 4).value
        print(hptv_name, lptv_name)
        
        add_hptv_name = ws.cell(idx+1, 5).value
        add_lptv_name = ws.cell(idx+1, 6).value
        # hptv_value = float(ws.cell(idx+1, 7).value)
        # lptv_value = float(ws.cell(idx+1, 8).value)
        
        hptv_arr = np.zeros_like(mr_arr)
        lptv_arr = np.zeros_like(mr_arr)
        
        oar_dict = {}
        for rcs, ssr in zip(rs.ROIContourSequence, rs.StructureSetROISequence):
            
            
            if hptv_name in str(ssr.ROIName):
                print(ssr.ROIName)
                for ctr_data in rcs.ContourSequence:
                    ...
                    hptv_arr[co_raster[0][2], co_filled_y, co_filled_x] = 1
            if lptv_name is not None: 
                if lptv_name in str(ssr.ROIName):
                    print(ssr.ROIName)
                    for ctr_data in rcs.ContourSequence:
                        co = np.array(ctr_data.ContourData)
                        co = np.reshape(co, (len(co)//3, 3))
                        ...
                        co_raster_y = co_raster[:, 1]
                        co_filled_y, co_filled_x = polygon(co_raster_y, co_raster_x)
                        lptv_arr[co_raster[0][2], co_filled_y, co_filled_x] = 1

            if add_hptv_name is not None:
                if add_hptv_name in str(ssr.ROIName):
                    for ctr_data in rcs.ContourSequence:
                        co = np.array(ctr_data.ContourData)
                        co = np.reshape(co, (len(co)//3, 3))
                        co_raster = np.int32(np.round((co - origin)/(spacing)))
        ...
        # hptv_arr = hptv_arr.astype("float32")
        # hptv_arr[hptv_arr != 0] = hptv_value
        hptv_itk = sitk.GetImageFromArray(hptv_arr)
        hptv_itk.SetSpacing(spacing)
        hptv_itk.SetOrigin(origin)
        # lptv_arr = lptv_arr.astype("float32")
        # if lptv_value is not None:
        #     lptv_arr[lptv_arr != 0]= lptv_value
        
        lptv_itk = sitk.GetImageFromArray(lptv_arr)
        lptv_itk.SetSpacing(spacing)
        lptv_itk.SetOrigin(origin)
        

        # 
        # mr_itk = resample(mr_itk, (3.0, 3.0, 3.0), (256, 256, 128), 0)
        # ct_itk = resample(ct_itk, (3.0, 3.0, 3.0), (256, 256, 128), -1024)
        original_origin = np.array(mr_itk.GetOrigin())
        original_spacing = np.array(mr_itk.GetSpacing())
        
        # mr_itk = resample_by_size(mr_itk, 256, 256, False)
        # ct_itk = resample_by_size(ct_itk, 256, 256, False)

        # mr_itk = sitk.Resample(mr_itk, dose_itk)
        # ct_itk = sitk.Resample(ct_itk, dose_itk)
        hptv_itk = sitk.Resample(hptv_itk, mr_itk)
        hptv_arr = sitk.GetArrayFromImage(hptv_itk)
        hptv_arr[hptv_arr < 0.5] = 0 
        hptv_arr[hptv_arr >=0.5] = 1
        hptv_itk = sitk.GetImageFromArray(hptv_arr)
        hptv_itk.CopyInformation(mr_itk)
        
        lptv_itk = sitk.Resample(lptv_itk, mr_itk)
        lptv_arr = sitk.GetArrayFromImage(lptv_itk)
        lptv_arr[lptv_arr < 0.5] = 0 
        lptv_arr[lptv_arr >=0.5] = 1
        lptv_itk = sitk.GetImageFromArray(lptv_arr)
        lptv_itk.CopyInformation(mr_itk)
        
        dose_itk = sitk.Resample(dose_itk, mr_itk)

        # ct_itk, _ = registration_for_mr(mr_itk, ct_itk, -1024)
        os.makedirs(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\HPTV", exist_ok=True)
        os.makedirs(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\LPTV", exist_ok=True)
        os.makedirs(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\MR", exist_ok=True)
        os.makedirs(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\CT", exist_ok=True)
        os.makedirs(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\DOSE", exist_ok=True)
        
        sitk.WriteImage(hptv_itk, os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\HPTV", "P%03d_" %(idx) + MRN + "_hptv.nii.gz"))
        sitk.WriteImage(lptv_itk, os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\LPTV", "P%03d_" %(idx) + MRN + "_lptv.nii.gz"))
        sitk.WriteImage(mr_itk, os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\MR", "P%03d_" %(idx) + MRN + "_mr.nii.gz"))
        sitk.WriteImage(ct_itk, os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\CT", "P%03d_" %(idx) + MRN + "_ct.nii.gz"))
        
        sitk.WriteImage(dose_itk, os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\DOSE", "P%03d_" %(idx) + MRN + "_dose.nii.gz"))
        
        
        BEAM_SAVEPATH = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\BEAM"
        os.makedirs(os.path.join(BEAM_SAVEPATH, "P%03d_" %(idx) + MRN), exist_ok=True)
        OAR_SAVEPATH = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\OAR"
        ...
        for key,val in oar_dict.items():
            ...
            oar_itk = sitk.Resample(oar_itk, mr_itk)
            oar_arr = sitk.GetArrayFromImage(oar_itk)
            oar_arr[oar_arr >=0.5]= 1
            oar_arr[oar_arr < 0.5]= 0
            oar_itk = sitk.GetImageFromArray(oar_arr)
            oar_itk.CopyInformation(mr_itk)
            
            sitk.WriteImage(oar_itk, os.path.join(OAR_SAVEPATH, "P%03d_" %(idx) + MRN, "P%03d_" %(idx) + MRN + "_%s.nii.gz" %(key)))
        
        print(hptv_arr.shape, lptv_arr.shape)
        ptv_original_arr = hptv_arr + lptv_arr
        ptv_original_arr[ptv_original_arr > 1] = 1
        
        import time

        
        ptv_original_arr = np.where(ptv_original_arr != 0, 1, 0).astype("uint8")
        for i in range(len(rtp.BeamSequence)):
            
                
            gantry_angle = rtp.BeamSequence[i].ControlPointSequence[0].GantryAngle
            # ptv_arr = binary_dilation(ptv_original_arr, iterations=3)
            ptv_arr = np.copy(ptv_original_arr)
            ...
            # leafjaw = np.array(rtp.BeamSequence[i].ControlPointSequence[0].BeamLimitingDevicePositionSequence[0].LeafJawPositions)
            source_to_col = rtp.BeamSequence[i].BeamLimitingDeviceSequence
            x_iso, z_iso, y_iso = isocenter
            ...
            origin = np.array(mr_itk.GetOrigin())
            spacing = np.array(mr_itk.GetSpacing())
            
            pix_iso = np.int32(np.round((isocenter - origin)/spacing))
            
            start_time = time.time()
            
            
            
            dose_ref_point = np.array(rtp.DoseReferenceSequence[1].DoseReferencePointCoordinates)
            # print(spa1[0], spacing[0])
            print(x_iso, y_iso, z_iso)
            x_beam = x_iso + 110 * (spacing[0]/original_spacing[0]) #+ SAD * np.sin(np.deg2rad(gantry_angle)) # beam 시작 x 좌표 
            y_beam = y_iso - SAD * (spacing[1]/original_spacing[1]) #* np.cos(np.deg2rad(gantry_angle)) # beam 시작 y 좌표 
            z_beam = z_iso #* spacing[2]/original_spacing[2]
            
            beam_source_xyz = np.array([[x_beam, y_beam, z_beam]])
            beam_source_ras = np.int32(np.round((beam_source_xyz - origin)/ spacing))
            # beam_source_ras_1 = np.int32(np.round((beam_source_xyz - ori1)/(spa1)))
            # beam_source_ras_back_1 = (beam_source_ras_1 * spa1) + ori1
            # print(beam_source_ras_back_1)
            
            # beam_source_ras_back_2 = (beam_source_ras * spacing) + origin
            # print(beam_source_ras_back_2)            
            pix_iso_x = pix_iso[0]
            pix_iso_y = pix_iso[1]
            pix_iso_z = pix_iso[2]
            
            image_center_x = ptv_arr.shape[2]//2
            image_center_y = ptv_arr.shape[1]//2 
            image_center_z = ptv_arr.shape[0]//2 
            
            shape_meta = ptv_arr.shape
            x_shift = pix_iso_x - image_center_x
            y_shift = pix_iso_y - image_center_y
            shift_pad_coeff = 200
            ptv_arr = np.pad(ptv_arr, ((0, 0), (shift_pad_coeff, shift_pad_coeff), (shift_pad_coeff, shift_pad_coeff)), mode= "constant", constant_values=0)
            ptv_arr = ptv_arr[:, 
                            shift_pad_coeff+y_shift:y_shift+shape_meta[1]+shift_pad_coeff, 
                            shift_pad_coeff+x_shift:x_shift+shape_meta[2]+shift_pad_coeff]
            
            ptv_arr = np.pad(ptv_arr, ((0, 0), (pad_coeff, pad_coeff), (pad_coeff, pad_coeff)), mode= "constant", constant_values=0)
            
            st = time.time()
            gantry_angle_radian = np.deg2rad(float(gantry_angle)) # z y x SI AP RL
            rotator_1 = Rotate(angle=(-1 * gantry_angle_radian, 0, 0))
            rotator_2 = Rotate(angle=(gantry_angle_radian, 0, 0))
            
            ptv_tensor = torch.tensor(ptv_arr[np.newaxis, ...]).to(device)
            ptv_tensor = rotator_1(ptv_tensor, padding_mode="zeros")
            ptv_arr = np.round(np.squeeze(ptv_tensor.detach().clone().cpu().numpy())).astype("uint8")
            print(time.time()-st)
            ...
            # area로 하면 문제 발생.. 
            ptv_projection = np.squeeze(np.sum(ptv_arr, axis = 1))
            ptv_projection[ptv_projection > 1] = 1
            ...
            #         area_max = area
            #         big_area_y = y 
            ptv_co = np.where(ptv_arr == 1)
            ptv_co_y_center = int(np.round(np.mean(np.array(ptv_co)[1])))
            big_area_co = np.where(ptv_projection == 1)
            
            big_area_z_co = big_area_co[0]
            big_area_x_co = big_area_co[1]
            big_area_y_co = np.zeros_like(big_area_co[0]) + ptv_co_y_center
            ...
            area_co_x[area_co_x > ptv_arr.shape[2]-1] = ptv_arr.shape[2]-1
            area_co_z[area_co_z > ptv_arr.shape[0]-1] = ptv_arr.shape[0]-1
            ...
            area_co_1 = area_co[:len(area_co)//2]
            area_co_2 = area_co[len(area_co)//2: 2 * len(area_co)//2]
            # area_co_3 = area_co[2 * len(area_co)//3: 3 * len(area_co)//3]
            # area_co_4 = area_co[3 * len(area_co)//4: 4 * len(area_co)//4]
            
            data_split = [[line_draw_arr, area_co_1, beam_source_ras, ptv_arr, MV, spacing],
                        [line_draw_arr, area_co_2, beam_source_ras, ptv_arr, MV, spacing],
                        # [line_draw_arr, area_co_3, beam_source_ras, ptv_arr, MV, spacing],
                        # [line_draw_arr, area_co_4, beam_source_ras, ptv_arr, MV, spacing]
                        ]
            ...
            line_draw_tensor = torch.tensor(line_draw_arr[np.newaxis, ...]).to(device)
            line_draw_tensor = rotator_2(line_draw_tensor, padding_mode="zeros")
            line_draw_arr = np.round(np.squeeze(line_draw_tensor.detach().clone().cpu().numpy())).astype("uint8")
            line_draw_arr = line_draw_arr[:, pad_coeff:arr_y_size + pad_coeff, pad_coeff:arr_x_size+pad_coeff]
            line_draw_arr = np.pad(line_draw_arr, ((0, 0), (shift_pad_coeff, shift_pad_coeff), (shift_pad_coeff, shift_pad_coeff)), mode= "constant", constant_values=0)
            line_draw_arr = line_draw_arr[:, 
                                        shift_pad_coeff-y_shift:shape_meta[1]+shift_pad_coeff-y_shift, 
                                        shift_pad_coeff-x_shift:shape_meta[2]+shift_pad_coeff-x_shift]
