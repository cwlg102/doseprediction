...
if __name__ == "__main__":
    ...
    netG = Model(in_ch=14, growth_rate=16, upsample_chan=64, out_ch=1).to(device)
    netG.load_state_dict(torch.load(r"X:\!project\!2025_doseprediction\doseprediction_modelG_139_1.597.pth")["netG_state_dict"])
    netG.eval()
    
    basepath = r"X:\!project\!2025_doseprediction\data\processed_data\traindata"
    ...
    savepath = r"X:\!project\!2025_doseprediction\results\exp05"
    os.makedirs(savepath, exist_ok=True)
    for idx, (mr_dir, beam_dir, hptv_dir, lptv_dir, oar_dir) in enumerate(zip(MR_dirs_list, BEAM_dirs_list, HPTV_dirs_list, LPTV_dirs_list, OAR_dirs_list)):
        ...
        hptv_arr /= 100 
        lptv_arr /= 100
        realA = torch.cat([mr_arr, beam_arr, hptv_arr, lptv_arr], dim=1)
        for oar in oar_list:
