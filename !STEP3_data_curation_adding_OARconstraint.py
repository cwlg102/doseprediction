...
xl_path = r"X:\!project\!2025_doseprediction\abdominal_dose_patients_cl_jk_update_OARconstraint.xlsx"

wb = openpyxl.load_workbook(xl_path)
ws = wb['Sheet']

savepath_site = r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_3\imagesTr_additional_info_site"
savepath_treat = r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_3\imagesTr_additional_info_treatment"
savepath_oarc = r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_3\imagesTr_additional_oar_constraint"
os.makedirs(savepath_site, exist_ok=True)
os.makedirs(savepath_treat, exist_ok=True)
os.makedirs(savepath_oarc, exist_ok=True)

im_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_3\imagesTr_MR"
...
for idx, im_dir in enumerate(im_dirs_list):
    ...
    oarc_arr = np.zeros_like(im_arr)
    
    if "(OARc)" in str(ws.cell(idx + 3, 4).value):
        oarc_arr += 1
    oarc_itk = sitk.GetImageFromArray(oarc_arr)
    oarc_itk.CopyInformation(im_itk)
    ...
    sitk.WriteImage(oarc_itk, os.path.join(savepath_oarc, im_dir.split("_")[0] + "_" + im_dir.split("_")[1] + "_oarconstraint.nii.gz"))