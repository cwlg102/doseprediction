...
if __name__ == "__main__":
    ...
    for idx in range(100):
        ...
        for rcs, ssr in zip(rs.ROIContourSequence, rs.StructureSetROISequence):
            if idx == 34:
                if "ptv" in str(ssr.ROIName).lower() or "target" in str(ssr.ROIName).lower():
                    print(ssr.ROIName)
        ...
        # sitk.WriteImage(ptv_itk, os.path.join(r"G:\! project\2024- UnityDosePrediction\data\processed_data_revised\PTV", "P%03d_" %(idx) + MRN + "_ptv.nii.gz"))
        # sitk.WriteImage(mr_itk, os.path.join(r"G:\! project\2024- UnityDosePrediction\data\processed_data_revised\MR", "P%03d_" %(idx) + MRN + "_mr.nii.gz"))
        # sitk.WriteImage(ct_itk, os.path.join(r"G:\! project\2024- UnityDosePrediction\data\processed_data_revised\CT", "P%03d_" %(idx) + MRN + "_ct.nii.gz"))
        
        # sitk.WriteImage(dose_new_itk, os.path.join(r"G:\! project\2024- UnityDosePrediction\data\processed_data_revised\DOSE", "P%03d_" %(idx) + MRN + "_dose.nii.gz"))
        
        BEAM_SAVEPATH = r"G:\! project\2024- UnityDosePrediction\data\processed_data_revised_1\BEAM"
        os.makedirs(os.path.join(BEAM_SAVEPATH, "P%03d_" %(idx) + MRN), exist_ok=True)
        # OAR_SAVEPATH = r"G:\! project\2024- UnityDosePrediction\data\processed_data_revised\OAR"
        ...
        for i in range(len(rtp.BeamSequence)):
            ...
            x_beam = x_iso + 110 #+ SAD * np.sin(np.deg2rad(gantry_angle)) # beam 시작 x 좌표 
            ...
            st = time.time()
            gantry_angle_radian = np.deg2rad(float(gantry_angle))
