...
mr_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\MR"
mr_dirs_list = sorted(os.listdir(mr_basepath))

ct_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\CT"
ct_dirs_list = sorted(os.listdir(ct_basepath))

beam_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\BEAM"
beam_dirs_list = sorted(os.listdir(beam_basepath))

oar_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\OAR"
oar_dirs_list = sorted(os.listdir(oar_basepath))

hptv_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\HPTV"
hptv_dirs_list = sorted(os.listdir(hptv_basepath))

lptv_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\LPTV"
lptv_dirs_list = sorted(os.listdir(lptv_basepath))

dose_basepath = r"X:\!project\!2025_doseprediction\data\processed_data\raw_data_2\DOSE"
dose_dirs_list = sorted(os.listdir(dose_basepath))

dcm_basepath = r"G:\! project\2024- UnityDosePrediction\data\dose_generation"
dcm_dirs_list = sorted(os.listdir(dcm_basepath))
dcm_plan_dirs_list = []
for i in range(len(hptv_dirs_list)):
    dcm_name = hptv_dirs_list[i].split("_")[1]
    nominate = []
...
def MakeBeamOnehot(mr_itk, beam_basepath, beam_fol_dir,dcm_basepath, dcm_plan_dir, res_size, res_spacing,idx):
    ds = pydicom.dcmread(os.path.join(dcm_basepath, dcm_plan_dir), force=True)
    
    beam_dirs = sorted(os.listdir(os.path.join(beam_basepath,beam_fol_dir)))
    beam_list = []
    for ref_beam, beam_dir in zip(ds.FractionGroupSequence[0].ReferencedBeamSequence, beam_dirs):
        dosegy = float(ref_beam.BeamDose)
        ...
        beam_dose_arr = np.where(beam_arr == 1, dosegy, 0)
        beam_list.append(beam_dose_arr.astype("float32"))
    ...
    beam_savepath = os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_from800downsample", "imagesTr_BEAM")
...
def GetOARdict(mr_itk, oar_basepath, oar_fol_dir, res_size, res_spacing, idx):
    ...
    oar_dirs = sorted(os.listdir(os.path.join(oar_basepath, oar_fol_dir)))
    ...
    for key, val in oar_name_dict.items():
        if key != "bowel":
            if val is None:
                oar_name_dict[key] = np.zeros_like(mr_arr).astype("uint8")
        else:
            if val is None:
                oar_name_dict[key] = np.zeros_like(mr_arr).astype("uint8")
            else:
    ...
    oar_savepath = os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_from800downsample", "imagesTr_OARs")
    ...
    return None
def GetHPTV(mr_itk,ptv_basepath, ptv_dir, dcm_basepath, dcm_plan_dir, res_size, res_spacing, idx):
    ds = pydicom.dcmread(os.path.join(dcm_basepath, dcm_plan_dir), force=True)
    ...
    ptv_savepath = os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_from800downsample", "imagesTr_HPTV")
    os.makedirs(ptv_savepath , exist_ok=True)
    sitk.WriteImage(ptv_itk, os.path.join(ptv_savepath, "%s" %ptv_dir))
    
    return None

def GetLPTV(mr_itk,lptv_basepath, lptv_dir, dcm_basepath, dcm_plan_dir, res_size, res_spacing, idx):
    ds = pydicom.dcmread(os.path.join(dcm_basepath, dcm_plan_dir), force=True)
    prescription = float(ds.DoseReferenceSequence[0].TargetPrescriptionDose)
    lptv_itk = sitk.ReadImage(os.path.join(lptv_basepath, lptv_dir))
    lptv_itk = resample(lptv_itk, res_spacing, res_size, 0)
    lptv_arr = sitk.GetArrayFromImage(lptv_itk).astype("float64")
    lptv_arr[lptv_arr >= 0.5] = 1 
    lptv_arr[lptv_arr < 0.5] = 0
    lptv_arr *= prescription
    lptv_itk = sitk.GetImageFromArray(lptv_arr.astype("float32"))
    lptv_itk.SetSpacing(mr_itk.GetSpacing())
    lptv_itk.SetOrigin(mr_itk.GetOrigin())
    lptv_itk.SetDirection(mr_itk.GetDirection())
    lptv_savepath = os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_from800downsample", "imagesTr_LPTV")
    os.makedirs(lptv_savepath , exist_ok=True)
    sitk.WriteImage(lptv_itk, os.path.join(lptv_savepath, "%s" %lptv_dir))
...
def GetDose(dose_basepath, dose_dir, res_size, res_spacing, idx):
    ...
    dose_savepath = os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_from800downsample", "labelsTr_DOSE")
...
if __name__ == "__main__":
    ...
    for idx, mr_dir, beam_fol_dir, oar_fol_dir, hptv_dir,lptv_dir, dose_dir, dcm_plan_dir in zip(
                                                            idx_list,
                                                            mr_dirs_list, 
                                                            beam_dirs_list, 
                                                            oar_dirs_list, 
        ...
        process_gethptv = Process(target=GetHPTV, args=(mr_itk, 
                                                      hptv_basepath, 
                                                      hptv_dir, 
                                                      dcm_basepath,
                                                      dcm_plan_dir,
        ...
        process_getlptv = Process(target=GetLPTV, args=(mr_itk, 
                                                      lptv_basepath, 
                                                      lptv_dir, 
                                                      dcm_basepath,
                                                      dcm_plan_dir,
        ...
        process_gethptv.start()
        process_getlptv.start()
        process_getdose.start()
        
        process_beamonehot.join()
        process_getoardict.join()
        process_gethptv.join()
        process_getlptv.join()
        ...
        print('saving...')
        mr_savepath = os.path.join(r"X:\!project\!2025_doseprediction\data\processed_data\traindata_128_from800downsample", "imagesTr_MR")
